...
# BEFORE ANYTHING ELSE - ORDER BY MODIFICATION DATE
# filelist = [ f for f in os.listdir(input_dir) if f.endswith(".JPG")]
# filetimes = []
# for i in filelist:
#     a = os.stat(os.path.join(input_dir,i))
#     filetimes.append(time.ctime(a.st_atime)) 

filelist = sorted(os.listdir(input_dir), key=os.path.getctime)
...
os.system("ls -1tr *.JPG | while read filename; do cp $filename renamed/$($counter)$filename;  done")
...
#     print the_text
os.system('Rscript order_by_time.R ' + input_dir)